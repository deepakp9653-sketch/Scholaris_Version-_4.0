...
export function CandidatePageClient({ batchId, initialData }: Props) {
  ...
  const handleExportExcel = () => {
    ...
    const rows = data.candidates.map((c: any, index: number) => [
      c.srNo || index + 1,
      `"${c.applicationId || ""}"`,
    ...
    link.setAttribute("download", `Cap_Candidate_Records_${new Date().toISOString().slice(0, 10)}.csv`);
  ...
  const handleExportPdf = () => {
    ...
    const rowsHtml = data.candidates
      .map(
        (c: any, index: number) => `
      <tr>
        ...
        <td style="padding: 6px; border: 1px solid #ddd;">${c.category || "—"}</td>
        <td style="padding: 6px; border: 1px solid #ddd;">${c.seatTypeCode || "—"}</td>
    ...
    const htmlContent = `
      ...
      <html>
        <head>
          <title>CAP Candidate Records Export</title>
          <style>
        ...
        <body>
          <div class="header">
            ...
            <h3>CAP Round Candidate Records Report</h3>
          </div>
          <table>
            <thead>
              <tr>
